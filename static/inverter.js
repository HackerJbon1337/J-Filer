/**
 * J-Filer — Color Inverter Frontend
 * Handles drag-and-drop, file management, inversion requests, and UI state.
 * Wired to /api/invert endpoint.
 */
...
(function () {
    ...
    const invertText = invertBtn.querySelector('.btn-merge-text');
    const invertLoad = invertBtn.querySelector('.btn-merge-loading');
    ...
    function addFiles(files) {
        ...
        files.forEach(file => {
            ...
            if (!ALLOWED_EXT.includes(ext)) {
                showToast(`Skipped "${file.name}" — only PDF & PPTX supported`, 'error');
                return;
            }
    ...
    function updateUI() {
        ...
        invertBtn.disabled = selectedFiles.length === 0;
    }
        ...
        try {
            const result = await new Promise((resolve, reject) => {
                ...
                xhr.upload.addEventListener('loadend', () => {
                    updateProgress(50, 'Inverting colors…');
                    let pct = 50;
                    progressInterval = setInterval(() => {
                        pct += Math.random() * 3 + 0.5;
                        if (pct > 90) pct = 90;
                        updateProgress(Math.round(pct), 'Processing pages…');
                    }, 500);
                });
                ...
                xhr.addEventListener('load', () => {
                    ...
                    if (xhr.status >= 200 && xhr.status < 300) {
                        ...
                        reader.onload = () => {
                            try { reject(new Error(JSON.parse(reader.result).error || 'Inversion failed')); }
                            catch { reject(new Error('Inversion failed')); }
                        };
                ...
                xhr.addEventListener('error', () => {
                    clearInterval(progressInterval);
                    reject(new Error('Network error during inversion'));
                });

                xhr.open('POST', '/api/invert');
            ...
            showToast(
                isBatch
                    ? `${selectedFiles.length} files inverted and zipped!`
    ...
    function setProcessing(loading) {
        invertBtn.disabled = loading;
        invertText.style.display = loading ? 'none' : 'inline';
        invertLoad.style.display = loading ? 'flex' : 'none';
    }
    ...
    function updateProgress(pct, stage) {
        ...
        progressTitle.textContent = pct >= 100 ? 'Inversion complete!' : 'Inverting colors…';
    ...
    function showSuccess(blob, filename, fileCount, isBatch) {
        ...
        const subtitle = isBatch
            ? `${fileCount} files inverted — packed into <strong>${escapeHtml(filename)}</strong> (${formatSize(blob.size)})`
            : `Inverted to <strong>${escapeHtml(filename)}</strong> (${formatSize(blob.size)})`;
        ...
        resultContent.innerHTML = `
            ...
            <div class="result-title" style="color: var(--accent-emerald);">Inversion Complete!</div>
            ...
            <div style="display:flex;gap:10px;flex-wrap:wrap;justify-content:center;">
                ...
                <button class="btn-new-merge" id="new-invert-btn">New Inversion</button>
            </div>
        `;
        ...
        document.getElementById('new-invert-btn').addEventListener('click', () => {
            clearAllFiles();
            resultCard.style.display = 'none';
        });
    ...
    function showError(message) {
        ...
        resultContent.innerHTML = `
            ...
            <div class="result-title" style="color:#f87171;">Inversion Failed</div>
